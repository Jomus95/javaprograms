...
public class Triangle {
    ...
    public float getEdgeSize(int edgeId) {
        if (edgeId < 1 || edgeId > 3) return -1;
        return (float) Math.sqrt(Math.pow(coords[(edgeId*2-2)] - coords[((edgeId%3+1)*2-2)],2)+
                                 Math.pow(coords[(edgeId*2-1)] - coords[((edgeId%3+1)*2-1)],2));
    }
